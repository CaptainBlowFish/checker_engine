...
class television:
    ...
    def __init__(self):
        ...
        self.screen = pygame.display.set_mode(
            (550, 400), pygame.RESIZABLE | pygame.SCALED
        )
        ...
        for width in range(self.game.board_size):
            ...
            for height in range(self.game.board_size):
                self._checker_board[width].append(
                    pygame.rect.Rect(
    ...
    def draw_pieces(self):
        for piece in self.game.get_all_pieces():
            if self.game.board[piece[0]][piece[1]]["black"]:
                ...
                self._pieces.append(
                    pygame.draw.circle(
                        self.screen, "red", (25 + piece[1] * 50, 25 + piece[0] * 50), 20
                    )
                )
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if self.game.black_win or self.game.red_win:
                    if pygame.Rect.collidepoint(
                        self._play_again_button.rect, mouse_x, mouse_y
                    ):
                        self.game.__init__()
            ...
            for column in range(len(self._checker_board[row])):
                if (
                    pygame.Rect.collidepoint(
                        self._checker_board[row][column], mouse_y, mouse_x
                    )
                    and [row, column] in self.game.movable_pieces
                ):
                    self._chosen_piece["row"] = row
                    self._chosen_piece["column"] = column
                elif pygame.Rect.collidepoint(
                    self._checker_board[row][column], mouse_y, mouse_x
                ):
                    self.game.move_piece(
                        self._chosen_piece["row"],
